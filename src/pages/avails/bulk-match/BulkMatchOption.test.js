...
import BulkMatchOption from './BulkMatchOption';
...
describe('EventManagementTable', () => {
    ...
    beforeEach(() => {
        wrapper = shallow(<BulkMatchOption selectedRights={selectedRights} />);
    });
    ...
    it('should be active and not show warning message when coreTitleId and sourceRightId is null for all rights and contentType is the same for all rights', () => {
        ...
        expect(wrapper.find('.nx-container-margin.table-top-text.nexus-c-bulk-match-option.active-link').length).toEqual(1);
    });
    ...
    it('should be disabled and show warning message when sourceRightId is not unique for each selected right', () => {
        ...
        expect(wrapper.find('.nx-container-margin.table-top-text.nexus-c-bulk-match-option').length).toEqual(1);
    });
